...
define("EW_RANDOM_KEY", "lv8u310luh6MPsD0", true);

$ewPathDelimiter = (strtolower(substr(PHP_OS, 0, 3)) === 'win') ? "\\" : "/";

$filepath = "phpmailer" . $ewPathDelimiter . "class.phpmailer.php";
...
function Send_Email($sFrEmail, $sToEmail, $sCcEmail, $sBccEmail, $sSubject, $sMail, $sFormat)
{
	...
	*/
	...
	$mail->IsSMTP(); 
	$mail->Host = EW_SMTPSERVER; // set smtp host
...
?>
<?php
define("HOST", $GLOBALS['configuration']['host']);
define("PORT", $GLOBALS['configuration']['port']);
define("USER", $GLOBALS['configuration']['user']);
define("PASS", $GLOBALS['configuration']['pass']);
define("DB", $GLOBALS['configuration']['db']);
